#!/bin/bash
# Run this script to stop all web apps within a subscription
...
while IFS=$'\t' read -r name resource_group; do
    ...
    echo "Stopping Web App: $name in Resource Group: $resource_group"
    az webapp stop --name "$name" --resource-group "$resource_group"
done <<< "$web_apps_info"